...
bool PhysBody::Contains(int x, int y) const
{
	...
	for (b2Fixture* f = body->GetFixtureList(); f && !ret; f = f->GetNext())
	{
		fixture.shape = f->GetShape();
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	b2Vec2 vect(PIXEL_TO_METERS(x1), PIXEL_TO_METERS(y1));
	b2Vec2 vect2(PIXEL_TO_METERS(x2), PIXEL_TO_METERS(y2));
	input.p1 = vect;
	input.p2 = vect2;
	input.maxFraction = 1;
	...
	for (b2Fixture* f = body->GetFixtureList(); f; f = f->GetNext())
	{
		...
		fixture.shape = f->GetShape();
		if (fixture.shape->RayCast(&output, input, body->GetTransform(), 0)){
	...
	if (closestFraction != 1) {
		b2Vec2 intersectionPoint = closestFraction * (vect2 - vect);

		normal_x = (intersectionNormal.x);
		normal_y = (intersectionNormal.y);
		int value = METERS_TO_PIXELS(sqrt(pow(intersectionPoint.x, 2) + pow(intersectionPoint.y, 2)));
		ret = value;
	}
	return ret;
}
...
void ModulePhysics::BeginContact(b2Contact* contact) {
	PhysBody* body1;
	body1 = new PhysBody;
	body1->body = contact->GetFixtureA()->GetBody();
	PhysBody* body2; 
	body2 = new PhysBody;
	body2->body = contact->GetFixtureB()->GetBody();
	
	if (body1->listener != NULL || body2->listener != NULL) {
